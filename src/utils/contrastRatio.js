function hexToRgb (hex) {
    ...
    const rgbArray = [parseInt(red, 16), parseInt(green, 16), parseInt(blue, 16)];
...
function contrast(mainRGB, contrastRGB) { 
    var contrastRatio = (luminance(mainRGB, mainRGB, mainRGB) + 0.05) / (luminance(contrastRGB, contrastRGB, contrastRGB) + 0.05); 
    if (contrastRatio < 1) contrastRatio = 1 / contrastRatio; 
    return contrastRatio;    
}

function getContrast(mainHex, contrastHex) {
    const contrastValue = parseFloat(contrast(hexToRgb(mainHex), hexToRgb(contrastHex)).toFixed(2));
